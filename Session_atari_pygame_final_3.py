...
def reward_processing(index, reward_in, cur_comp, reward_temp, reward_stage_lst, reward_stage_lst_MH, order,
    ...
    if index == 1:
        ...
        if order > 1:
            ...
            if len(reward_stage_lst_MH) > 0:
                prev_reward_MH = reward_stage_lst_MH[-1][-1]  # [-1][0]
                reward_stage_lst_MH.pop()
                print(reward_stage_lst_MH)
            else:
                prev_reward_MH = 0
            # prev_reward_MH = reward_stage_lst_MH[-1][-1]

            compare = str(cur_comp)
            if compare[0] == '1':
                if prev_reward_MH == 0:
                    reward_curr = np.nan
                    reward_stage_lst_MH.append([prev_reward_MH, reward_curr, reward_close])
                else:
                    reward_curr = reward_close / prev_reward_MH
                    reward_stage_lst_MH.append([prev_reward_MH, round(reward_curr), reward_close])
                ...
                if reward_close == 0:
                    ...
                    reward_curr = prev_reward_MH / reward_close
                    reward_stage_lst_MH.append([prev_reward_MH, round(reward_curr), reward_close])
            elif compare[0] == '3':
                if reward_close < 0:
                    reward_close_mod = -reward_close
                    reward_curr = prev_reward_MH + reward_close_mod ** (1 / 3)
                else:
                    reward_curr = prev_reward_MH - reward_close ** (1 / 3)
                reward_stage_lst_MH.append([prev_reward_MH, round(reward_curr), reward_close])

        elif cur_comp < 2:
            reward_temp += reward_close
            reward_stage_lst.append(reward_close)
            reward_stage_lst_MH.append([reward_close])

        else:
            reward_stage_lst.append(0)
            reward_stage_lst_MH.append([0])
    ...
    return reward_temp, prev_reward, reward_stage_lst, reward_stage_lst_MH
...
def openAI_function_atari(PART_NUMBER, SESS_NUMBER, ns=None,
    ...
    if SESS_NUMBER == 0:
        ...
        schedule_index_uncertainty = (3 * PART_NUMBER + session) % len(SESSs["uncertainty_list"]) - 2  # TODO : -2 없애기
        ...
        for block in schedule_env:
            ...
            while t_block.getTime() <= 60 * run_time_min:  # 8 min
                ...
                if ns is not None:
                    ...
                    ns.send_event('epi' + str(i_episode), label="episode", timestamp=egi.ms_localtime())  # TODO : egi.ms_localtime() t_ep.getTime()
                ...
                while True:  # sampling
                    ...
                    if gt > (slowing / FrameRate):  # render 60Hz   #0.01694915
                        ...
                        action, info_key, info_keys = key_processing(ns, env_name, keyss, action_space, info_key,
                                                                     info_keys, t, t_ep, pp, cur_comp, order) #TODO : t3

                        observation, reward, done, info = env.step(action)
                        ots.append(t_ep.getTime())  # observation time point / block마다 reset # TODO : t3
                        ...
                        if t_stage.getTime() >= stage_time:
                            ...
                            reward_temp, prev_reward, reward_stage_lst, reward_stage_lst_MH = reward_processing(0,
                                                                                                                reward_stage,
                                                                                                                cur_comp,
                                                                                                                reward_temp,
                                                                                                                reward_stage_lst,
                ...
                reward_temp, prev_reward, reward_stage_lst, reward_stage_lst_MH = reward_processing(1, reward_close,
                                                                                                    cur_comp,
                                                                                                    reward_temp,
                                                                                                    reward_stage_lst,
                                                                                                    reward_stage_lst_MH,
                ...
                if ns is not None:
                    ns.send_event('reward_epi' + str(i_episode), label="reward", timestamp=egi.ms_localtime()) # TODO : egi.ms_localtime()
                ...
                # schedule_goal.append(goal_conds) # 10-15초에 한 번 씩 바뀔 때마다 append
            ...
            # reward 보여줄 때 event tagging
            if ns is not None:
                ns.send_event('reward_block' + str(block_num), label="total_r", timestamp=egi.ms_localtime()) # TODO : egi.ms_localtime()
